...
- plot statistic parallel coordinates plots by using [`ggparcoord()`](https://ggobi.github.io/ggally/reference/ggparcoord.html) of [**GGally**](https://r4va.netlify.app/chap15) package, and
- plot interactive parallel coordinates plots by using [**parallelPlot**](https://cran.r-project.org/web/packages/parallelPlot/) package.

## 5.2 Installing and Launching R Packages
For this chapter, the **GGally, parallelPlot** and **tidyverse** packages will be used.
...
In this chapter the [World Happinees 2018](http://worldhappiness.report/ed/2018/) data will be used. The original data set is in Microsoft Excel format. It has been extracted and saved in csv file called **WHData-2018.csv**.
...
In this section, we will plot static parallel coordinates plot by using `ggparcoord()` of **GGally** package. Before getting started, read the function description in detail.
...
The basic parallel coordinates failed to reveal any meaning understanding of the World Happiness measures. In this section, we will makeover the plot by using a collection of arguments provided by `ggparcoord()`.
```{r}
ggparcoord(data = wh, 
           columns = c(7:12), 
           groupColumn = 2,
