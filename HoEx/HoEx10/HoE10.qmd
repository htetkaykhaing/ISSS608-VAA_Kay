...
For the purpose of this chapter, the following R packages will be used.
```{r}
...
::: {.callout-note}
Before running the code chunk, we need to change the R system to 32bit version. This is because the `odbcConnectAccess()` is based on 32bit and not 64bit
:::
...
In this section, we will prepare a bullet chart report by using functions of gt and gtExtras packages.
```{r}
#| code-fold: true
product %>%
  gt::gt() %>%
...
It is important to note that one of the requirement of gtExtras functions is that almost exclusively is required to pass data.frame with list columns. In view of this, code chunk below will be used to convert the report data.frame into list columns.
```{r}
#| code-fold: true
report %>%
  group_by(Product) %>%
...
Next, we all need to load the package onto R environment by using the code chunk below.
```{r}
